...
class Raven {
  ...
  update(deltaTime) {
    if (this.x < 0 || this.y > canvas.height - this.height) {
...
window.addEventListener("click", (e) => {
  ...
  const pixelColor = detectPixel.data;
  ...
  ravens.forEach((raven) => {
    if (
      ...
      if (pixelColor[0] === raven.randomColors[0]) {
        if (pixelColor[1] === raven.randomColors[1]) {
          if (pixelColor[2] === raven.randomColors[2]) {
            score++;
